...
2. **點擊作品**：其他作品淡出，被點的那顆游到畫面上方放大、發光、只留編號；
   ...
   進度條、後退 15 秒／播放／前進 15 秒。
3. **底部箭頭**（點一下或往上拉）：面板展開成完整逐字稿，目前句子反白並自動捲動。
...
- 首頁 10 個水波紋單元由一個輕量物理模擬驅動：基準大小統一（`UNIT_SIZE`），各自在 0.5~1.0 倍之間
...
1. iPad 連上網路，用 **Safari** 開 https://artogo.github.io/rtmss/ 。
2. 停留在頁面直到畫面下方出現「**已完成離線安裝，沒有網路也能使用**」的提示（約幾秒到十幾秒，視網速）。
3. 點 Safari 的「分享」→「**加入主畫面**」→「新增」。主畫面會出現「渡口回聲」圖示。
4. 從主畫面點開圖示（全螢幕、沒有網址列）。此時可以關閉 Wi-Fi 測試：所有作品都應該能播放。
5. 展場建議設定：
   - 設定 → 螢幕顯示與亮度 → **自動鎖定：永不**。
...
- **單元動態**：`src/hooks/useUnitPhysics.js` 檔頭的 `P`：`wanderAmp`/`wanderPeriod` 漂移範圍與週期、
  `scaleRange` 呼吸區間（預設 0.5~1.0）、`breathPeriod`/`breathPeriod2` 兩層呼吸週期（每顆不同，由編號決定）、
...
- 內頁前一則／下一則切換；閒置一段時間自動回首頁（展場公用裝置）。
- 加入書籤／已聽完標記。
- 多語言（中／英）切換。
